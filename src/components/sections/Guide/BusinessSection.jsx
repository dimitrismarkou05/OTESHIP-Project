...
const BusinessSection = () => {
  ...
  const { entrepreneurshipStepsData } = useLanguageData(); // State for the dynamic image

  const [randomImage, setRandomImage] = useState("");
  const [isLoaded, setIsLoaded] = useState(false); // Fetch the random image on mount
  ...
  return (
    <section
      className="flex flex-col gap-8 xs:gap-10 min-h-fit scroll-mt-24 md:scroll-mt-32 w-full transition-colors duration-200"
      id="entrepreneurship"
    >
      ...
      <div className="flex flex-col xl:flex-row gap-6 xs:gap-8 xl:items-stretch w-full">
        ...
        <div className="flex flex-col items-start gap-4 xs:gap-5 md:gap-6 flex-[1.5]">
          ...
          <p
            className="text-xs xs:text-sm md:text-base text-(--color-bg-dark) dark:text-(--color-bg-primary) leading-relaxed transition-colors duration-200"
            data-aos="fade-up"
            data-aos-delay="100"
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 xs:gap-4 w-full mt-2">
            ...
            <div
              className="bg-white dark:bg-(--color-dark-text) p-4 xs:p-5 rounded-md drop-shadow-sm/10 border border-(--color-divider)/40 flex flex-col gap-2 transition-colors duration-200"
              data-aos="fade-up"
              data-aos-delay="200"
            >
              <div className="flex items-center gap-2 text-(--color-primary)">
                <i className="fa-solid fa-lightbulb text-sm xs:text-base"></i>
                <h2 className="font-semibold text-(--color-dark-text) dark:text-white text-xs xs:text-sm transition-colors duration-200">
                  {t("business.understandingTitle")}
                </h2>
              </div>
              <p className="text-[11px] xs:text-xs md:text-sm text-(--color-bg-dark) dark:text-(--color-bg-primary) leading-relaxed transition-colors duration-200">
            ...
            {/* Forms Concept */}
            <div
              className="bg-white dark:bg-(--color-dark-text) p-4 xs:p-5 rounded-md drop-shadow-sm/10 border border-(--color-divider)/40 flex flex-col gap-2 md:col-span-2 transition-colors duration-200"
              data-aos="fade-up"
              data-aos-delay="400"
            >
        ...
        <Link
          to="/ceramics"
          className="relative rounded-md w-full xl:w-2/5 shrink-0 block overflow-hidden drop-shadow-lg h-56 xs:h-64 md:h-80 xl:h-auto xl:self-stretch bg-gray-200 dark:bg-gray-800 transition-colors duration-200"
          data-aos="zoom-in"
          data-aos-delay="300"
        >
          <div
      ...
      >
        <div className="flex flex-col gap-1.5">
          <h2 className="font-semibold text-lg xs:text-xl text-(--color-dark-text) dark:text-white transition-colors duration-200">
            {t("business.stepsTitle")}
          </h2>
          <p className="text-xs xs:text-sm md:text-base text-(--color-bg-dark) dark:text-(--color-bg-primary) border-b border-(--color-divider)/50 pb-2 transition-colors duration-200">
            {t("business.stepsDesc")}
          </p>
        </div>
        {/* Responsive Table Container */}
        <div className="overflow-x-auto bg-white dark:bg-(--color-dark-text) rounded-md drop-shadow-lg border border-(--color-divider)/50 mt-1 xs:mt-2 transition-colors duration-200">
