...
const LawSection = () => {
  ...
  return (
    <section
      className="flex flex-col items-start gap-6 xs:gap-8 md:gap-10 scroll-mt-24 md:scroll-mt-32 bg-white drop-shadow-lg rounded-md p-5 xs:p-6 md:p-8 lg:p-10"
      id="legislation"
    >
      ...
      <div className="flex flex-col gap-4 xs:gap-5 md:gap-6" data-aos="fade-up">
        <div className="flex flex-row items-start justify-start gap-2 xs:gap-2.5">
          <div className="bg-(--color-primary)/20 w-8 h-8 xs:w-10 xs:h-10 flex items-center justify-center text-(--color-primary) rounded-md shrink-0 mt-0.5 xs:mt-1 md:mt-1.5">
      ...
      <div
        ...
        className="w-full flex flex-col gap-3 xs:gap-4 md:gap-5 bg-(--color-bg-primary) drop-shadow-sm/10 p-4 xs:p-5 md:p-6 outline-1 outline-(--color-light3-text)/40 rounded-md"
      >
        {/* Changed items-center to items-start for the Globe icon */}
        <div className="flex flex-row items-start justify-start gap-2">
          <i className="fa-solid fa-globe text-(--color-primary) text-sm xs:text-base mt-0.5 xs:mt-1"></i>
          <h1 className="font-medium text-sm xs:text-base md:text-lg text-(--color-dark-text) dark:text-white leading-snug">
            {t("law.unRightsTitle")}
          </h1>
        </div>
        <div className="flex flex-col text-xs xs:text-sm md:text-base text-(--color-bg-dark) dark:text-(--color-bg-primary) ml-1">
      ...
      >
        <div className="flex flex-row border-b border-(--color-divider) overflow-x-auto overflow-y-hidden no-scrollbar">
          {["greece", "poland", "turkey"].map((country) => (
            <button
              key={country}
              onClick={() => setActiveTab(country)}
              className={`px-3 py-2 xs:px-4 xs:py-2.5 font-medium text-xs xs:text-sm md:text-base transition-colors relative cursor-pointer whitespace-nowrap ${
                activeTab === country
                  ? "text-(--color-primary)"
        ...
        {/* Dynamic Content */}
        <div className="bg-white p-4 xs:p-5 md:p-6 rounded-md outline-1 outline-(--color-light3-text)/40 drop-shadow-sm/5">
